...
center_front = FingerSensorConfig(np.array([x/2,0,-z/2+0.005],np.array([1,0,0])))

@dataclasses.dataclass
class TactilePickConfig:
    object_list: List[ObjectConfig] = None 
    sensor_config: List[FingerSensorConfig] = None
...
DEFAULT_CONFIG = TactilePickConfig(
        object_list = [ObjectConfig( str(ASSET_PATH / "cylinder" / "1:2cylinder.urdf"),.03, 0.05)],
        action_space = _4DOF_EEF_ACTION_SPACE(),
        sensor_config=[center_front]
        )
...
class TactilePick(gym.Env):
    ...
    def visualize_sensors(self):
        for sensor in self.config.sensor_config:
            position_in_finger_frame = sensor.position_in_finger_frame
            position_in_world_frame  =None
    ...
    def _clip_robot_state(self,state: np.ndarray):
        ...
        ranges = [self.config.pick_workspace_x_range,self.config.pick_workspace_y_range,self.config.workspace_z_range,self.config.workspace_euler_x_range,
        self.config.workspace_euler_y_range,self.config.workspace_euler_z_range,(0,1)]
